...
```{r}
#7 Combine the four datasets using 'rbind'. Double checked that column names are the same.
EPAair_18_19 <- rbind(EPAair_03_NC2018, EPAair_03_NC2019, EPAair_PM25_NC2018, EPAair_PM25_NC2019)

#8 Wrangle the new dataset with a pipe function (%>%). Include the 13 sites listed in the instructions. Add columns for "Month" and "Year" by parsing your "Date" column.
...
#9 Spread your datasets such that AQI values for ozone and PM2.5 are in separate columns. Each location on a specific date should now occupy only one row.
EPAair_18_19_spread <- pivot_wider(EPAair_18_19_wrangle, names_from = AQS_PARAMETER_DESC, values_from = AQS_PARAMETER_DESC)

EPAair_18_19_tidy <- EPAair_18_19_spread 

#10 Call up the dimensions of your new tidy dataset.
dim(EPAair_18_19_tidy)

#11 Save the processed dataset with the following file name: "EPAair_O3_PM25_NC1819_Processed.csv".
write_csv(EPAair_18_19_tidy, file = "./Data/Processed/EPAair_03_PM25_NC1819_Processed.csv")
...
```{r}
#12a Use the split-apply-combine strategy to generate a summary data frame. Group data by site, month, and year. Generate the mean AQI values for ozone and PM2.5 for each group.
EPAair_18_19_summaries <- 
  EPAair_18_19_tidy %>% 
  group_by(Site.Name, Month, Year) %>% 
  summarise(meanAQI, Ozone, PM2.5)

#12b Add a pipe to remove instances where a month and year are not available (use the function `drop_na` in the pipe). 
