...
#Temperature boxplot
boxplotA <-
  ggplot(PeterPaul.chem.nutrients, aes(x = factor(month, levels = c(1:12)), y = temperature_C)) + #Convert month integers into factors and include factors 1-12 in the output
...
boxplotB <-
  ggplot(PeterPaul.chem.nutrients, aes(x = factor(month, levels = c(1:12)), y = tp_ug)) + #Convert month integers into factors and include factors 1-12 in the output
  xlab(expression("Month")) + #Set axis labels
...
boxplotC <-
  ggplot(PeterPaul.chem.nutrients, aes(x = factor(month, levels = c(1:12)), y = tn_ug)) + #Convert month integers into factors and include factors 1-12 in the output
  xlab(expression("Month")) + #Set axis labels
...
#Make a cowplot with the three graphs - boxplotA, boxplotB, boxplotC. Make sure there is only one legend and that the axis are aligned
combined_plotsABC <-
plot_grid(boxplotA + theme(legend.position = "none"), boxplotB +  theme(legend.position = "none"), boxplotC + theme(legend.position = "bottom"), nrow = 1, align = 'hv', axis = "bt", rel_heights = c(1.6, 1.7, 2.3))
...
drymass_by_date <-
  ggplot(NiwotRidge.litter[NiwotRidge.litter$functionalGroup %in% "Needles",], aes(x = collectDate, y = dryMass)) + #Display the "Needles" functional group the litter dataset
