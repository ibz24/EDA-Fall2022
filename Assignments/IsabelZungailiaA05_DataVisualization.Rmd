...
totalphos.vs.phosphate <-
  ...
  ylab(expression("Total Phosphorus (tp_ug)")) +
...
boxplotC <-
  ...
  xlab(expression("Month")) +
  ylab(expression("Total Nitrogen (tn_ug)")) +
  guides(color=guide_legend(title="Lake Name")) +
  geom_boxplot(aes(color = lakename)) +
  scale_x_discrete(drop = FALSE)
print(boxplotC)

#Combine 3 boxplots into 1 using plot_grid'
plot_grid(boxplotA, boxplotB, boxplotC, nrow = 3, align = 'hv', axis = "bt", rel_heights = c(3, 3, 3))

plot_grid(boxplotA + theme(legend.position = "none", axis.title.x = element_blank()), boxplotB +  theme(legend.position = "none", axis.title.x = element_blank()), boxplotC + theme(legend.position = "bottom", axis.title.x = element_blank()), nrow = 3, align = 'hv', axis = "bt", rel_heights = c(1.6, 1.7, 2.3))
...
combinedplots <-
  plot_grid(boxplotA, boxplotB, boxplotC)
print(combinedplots)
  
#Figure out how to combine legends into 1. 
