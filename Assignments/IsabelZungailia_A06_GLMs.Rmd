...
> Answer: This figure suggests that temperature (C) decreases as depth increases (m). The points are relatively evenly distributed on either side of the trend line, which suggests that this trend is linear. 
...
#The residual error refers to the remainder of variance not explained by the model. In this case, it was 1.694 on 9726 degrees of freedom (the number of variables we are considering in the sample).
#Temperature is predicted to change ___ for every 1m change in depth. 
...
LakeTemp.by.depth <-
  ggplot(NTL_LTER.RAW.chem.phys.wrangled, aes(x = depth, y = temperature_C)) +
  geom_point(aes(color = lakename, shape = lakename)) + #Separate color for each lake, points 50% transparent
