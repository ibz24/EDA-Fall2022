...
```{r scatterplot}
#4 
#Wrangle NTL-LTER dataset with a pipe function
wrangled_NTL_LTER.chem.phys <-
  NTL_LTER.chem.phys %>%
  filter(month = July) %>% #Only dates in July
  select(lakename, year4, daynum, depth, temperature_C) %>% #Select specific columns
  drop_na(NTL_LTER.chem.phys) *Remove NAs so that only complete cases exists
...
temperature.by.depth <-
  ...
  ylab(expression("Temperature")) + #Change y-axis label
