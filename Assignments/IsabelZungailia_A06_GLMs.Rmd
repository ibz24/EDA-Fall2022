...
> Answer: This figure suggests that temperature (C) decreases as depth increases (m). There is a negative correlation between temperature and depth in the Month of July. The points are relatively evenly distributed on either side of the trend line, which suggests that this trend is linear. [Research further]
...
NTL_Corr <- cor(NTL_subset)
view(NTL_Corr) #Hard to visualize, can use "ellipse" to get a better visualization
corrplot(NTL_Corr, method = "ellipse")
...
> Answer: 
The AIC method suggests that we use year4, daynum, and depth to predict temperature in our multiple regression. In our model, the r-squared value is 0.7411, which means that the model closely decsribes the variance between the variables (74.11% of observed variance is explained). The overall p-value is 2.2e-16 (less than 0.05), which means that there is a strong relationship between the variables. Residual standard error is 3.817 on 9724 degrees of freedom. There is a slight (about 1%) improvement over the model using only depth as the explanatory variable.
---
...
```{r anova.model}
#12
July_Lake_Temps.anova <- #One-way ANOVA test using aov()
  aov(data = NTL_LTER.RAW.chem.phys.wrangled, temperature_C ~ lakename)
summary(July_Lake_Temps.anova)

July_Lake_Temps.anova2 <- #One-way ANOVA test using lm()
...
> Answer: The results of the ANOVA tests indicated that there is a signifcant difference in the mean temperature (C) among the lakes. The p-value from ANOVA is <2e-16 (much less than 0.05), which means that we reject the null hypothesis and we can assume that there are significant differences between the groups in question. The r-squared value indicates that around 4% of the variability in the mean temperature among the lakes is explained by this model. 
...
#14.
LakeTemp.by.depth <-
  ggplot(NTL_LTER.RAW.chem.phys.wrangled, aes(x = depth, y = temperature_C, color = lakename)) + #Separate color for each lake
  geom_point(alpha = 0.5) + #Make point 50% transparent
...
July_Lake_Temps.Groups <-
 HSD.test(July_Lake_Temps.anova, "lakename", group = TRUE)
July_Lake_Temps.Groups
...
>Answer: The findings indicate that Paul Lake and Ward Lake were the only two lake lakes that have the same mean temperature (statistically speaking) as Peter Lake. None of the lakes have a mean temperature that is statistically distinct from all the other lakes.

 

17. If we were just looking at Peter Lake and Paul Lake. What's another test we might explore to see whether they have distinct mean temperatures? 

>Answer: We could use the HSD.test to extract groupings for pairwise relationship and explore whether the lakes have distinct mean temperatures.
...
wrangled.Crampton_Ward <-
  NTL_LTER.RAW.chem.phys.wrangled %>% 
  filter(lakename == "Crampton Lake" | lakename == "Ward Lake") #Only include records for Crampton Lake and Ward Lake
...
>Answer: The mean temperature (C) in Crampton Lake is 15.35189 and the mean temperature for Ward Lake is 14.45862. The p-value of the two-sample T-test is 0.2649 (greater than 0.05), which means that we fail to reject the null hypothesis and can conclude that the means are similar. 
