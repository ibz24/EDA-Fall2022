...
>Answer: This plot suggests that that ozone concentrations remain relatively stable over time, despite fluctuations throughout the seasons. 
...
#Fill in missing daily data for AQI values
GaringerOzone2$DAILY_AQI_VALUE <- na.approx(GaringerOzone2$DAILY_AQI_VALUE)
summary(GaringerOzone2$DAILY_AQI_VALUE)
```

> Answer: Interpolation is a common method used to replace missing values in a dataset. It is important to note that we want to replace the missing values through interpolation - we do NOT want to remove them completely. Interoplation will not change the main properties of the distribution (min, max, mean, etc). Linear interpolations are a "connect the dots" approach - missing data values are assumed to fall between the previous and next measurement. A straight line is drawn between the two predetermined values. 
...
GaringerOzone.monthly <- 
  ...
  group_by(month_year) %>% 
  summarize(mean_monthly_03 = mean(Daily.Max.8.hour.Ozone.Concentration)) #Mean ozone concentrations
...
GaringerOzone.monthly.ts <- ts(GaringerOzone.monthly$mean_monthly_03, start = c(2010, 01),
frequency = 12)
...
GaringerOzone_trend
#The p-value 0.046724 P-value is only a little significant...
...
MeanMonthlyOzone.plot <- ggplot(GaringerOzone.monthly, aes(x = month_year, y = mean_monthly_03)) +
  geom_point() +
  geom_line() +
...
> Answer: The research question asked: Have ozone concentrations changed over the 2010s at this station? The p-value 0.046724 P-value is only a little significant...
...
GaringerOzone.monthly.decomposed <- stl(GaringerOzone.monthly.ts, s.window = "periodic")
plot(GaringerOzone.monthly.vdecomposed)

#16. Run the Mann Kendall test on the non-seasonal Ozone monthly series
GaringerOzone.monthly.ts2 <- ts(GaringerOzone.decomposed$mean_monthly_03, start = c(2010, 01), frequency = 12)

GaringerOzone_trend2 <- Kendall::SeasonalMannKendall(GaringerOzone.monthly.ts2)
