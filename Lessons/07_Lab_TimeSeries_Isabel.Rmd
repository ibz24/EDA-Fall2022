...
Temperature <-
  ...
  labs(x = "Date", y = "Temperature (C)") +
  geom_smooth()
  #can see a trend and seasonality
...
#Optional code...instead of typing in the specific year and month in the code below
f_month <- month(first(BRA_complete$Date))
f_year <- year(first(BRA_complete$Date))


Rainfall.ts <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)
Rainfall.ts

Temperature.ts <- ts(BRA_complete$Temperature_C, start = c(1901,1), frequency = 12)
Temperature.ts
```
...
plot(Rainfall_decomposed)
  #Seasonality plays an important role in the variable

Temperature_decomposed <-stl(Temperature.ts, s.window = "periodic")
plot(Temperature_decomposed)
  #Both trend and seasonality have the same relative influence on the variable
...
Rainfall_components <- mutate(Rainfall_components, Observed = BRA_complete$Rainfall, Date = BRA_complete$Date)

Temperature_components <- mutate(Temperature_components, Observed = BRA_complete$Temperature_C, Date = BRA_complete$Date)
