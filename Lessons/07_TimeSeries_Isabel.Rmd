...
#First step is to always plot the data...
#Gap in data :1971-1985
```
...
#Split dataset and perform separate analysis
```
...
#(year, month), and the frequency of the data
```
...
plot(EnoDischarge.late_Decomposed)
    ...
    #Plots the seasonal component, trend component, and the random component
    #Bar (on the right sides of the plots)represents the variability of the datasets
    #Larger the gray bar, the smaller the variability of the component in the dataset (seasonal component is bigger than the trend component) 
    #The smaller the bar, the higher the representedness of that variable in your data
    #Remainder is the hardest variable to forecast...
...
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3])
  #the object 'time series' has seasonal, trend, remainder columns

#Changing some columns-->add column with observed data and a column with the date object
...
ggplot(EnoDischarge.late_Components) +
  ...
  #red line shows how trend varies over time
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))
  #seasonal compoent goes below y=0 line in every cycle of the seasonal component; be careful with this!
...
The Eno River discharge data doesn't have any short periods of missing data, so interpolation would not be a good choice for that dataset. We will illustrate a linear interpolation using the wind speed dataset. [there is one NA in the last column]
...
wind_data_clean <- 
  ...
  mutate( AWND.clean = zoo::na.approx(AWND) )
    #use the zoo package
    #na.approx. function used to interpolate missing values that exist in the middle of the dataset
    #interpolation won't change main properties of the distribution (min, max, mean, etc)
...
ggplot(wind_data_clean ) +
  ...
  #connected the dots and created a continuous observation (replace the missing values with interpolation, not removing them!)

```

### Monotonic trend analysis, continued
  #always upward or always downward
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall) (only test that can be used for seasonal data...)
...
```{r}
#decompose series, remove seasonal components, then run the time series test
#there is also something called a Seasonal Mann-Kendall test (breaks data down into seasons and draws comparisons)
#doesn't always give the same results...
#Mann-Kendall is not good for detecting stochastic trends (use ADF instead) --> always start with the ADF to see if a stochastic trend exists (and if it doesn't, use the Mann-Kendall)

#For time series, we need to convert data frames to time series objects...but then convert back to data frame for visualization
#To interpret Mann-Kendall: if s is positive, trend is increasing; if s is negative, trend is decreasing 

# Generate time series (trend test needs ts, not data.frame)
f_month <- month(first(wind_data_clean$DATE)) #first month on the first observation for this date
f_year <- year(first(wind_data_clean$DATE)) #first year on the first observation of the date
...
#if p-value is less than 0.05, we can reject null hypothesis and say that we have a trend

wind_data_trend2 <- trend::smk.test(wind_data_ts) #another way to run the test...
# Inspect results
wind_data_trend2
...
#plot wind data over time and add a linear regression; decreased trend in average wind speec over time
