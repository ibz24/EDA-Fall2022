...
#View using mapview - reads spatial data frames and pairs them with a nice backdrop/pulls in the attributes
mapview(GasPlants)
...
class(EPAair.sf)
#Long and lat become "geographic coordinates" column in the dataframe 
...
sfg_GraingerHall <- st_point(c(-78.9427,36.00465)) #this is a simple feature point
class(sfg_GraingerHall)
mapview(sfg_GraingerHall) #shows up as a point if

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #create a simple feature column and link it with a coordinate reference system (4326) (ties the coordinate with a locatio on earth)
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe
sf_GraingerHall <- st_as_sf(sfc_GraingerHall) #upgrade using 'st_as_sf' to associate it with attributes
...
A bit odd, no? Why is the circle so big? What is the units of '1'? And why is it oval, not a circle? This has much to do with the fact that the geometric units of our feature are measured in angular units (degrees) and not familiar planar ones (like meters or feet). And that's because latitude and longitude measure location on a sphere, and we like doing
calculations on a plane...

We must reconfigure our datasets so that they come out well on a plane...
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #changes coordinates to eastings and northings from the center coordinates (linear units - meters)
...
#line has a little bit of a bend in it...some distortion
#For each EPA site, calculate the mean distance to all the others (is there a relationship between distance and air quality)
...
#pairwise means that we want to create a matrix

#Compute the mean of each row (each EPA site)
mean_distances <-  rowMeans(distance_matrix) #collapse all columns to just have the mean

#Add as a column to the epa sites
EPAair.sf.utm  <-  EPAair.sf.utm %>% 
  bind_cols(mean_distances)

#add mean values column to the EPA dataset
