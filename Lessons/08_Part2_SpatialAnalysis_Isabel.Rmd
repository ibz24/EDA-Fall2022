...
epa_pm25_sites_sf <- read_csv('./Data/Raw/EPAair_PM25_NC2018_raw.csv') %>% 
  ...
  summarize(   #Collapse by site and calculate mean and max for each site
    meanPM = mean(`Daily Mean PM2.5 Concentration`,na.rm=T),
    maxPM = max(`Daily Mean PM2.5 Concentration`,na.rm=T)
    ) %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) #convert into a spatial features object (specify x and y values)
...
#4269 - NAD 1983, geographic coordinate system (unit = degrees)
...
#we can filter for a specific state (this is census data)
#NC is FP37
#these will be polygons (2 coordinate pairs)
...
#
nrow(counties_sf) #100 rows

#Reveal the extent of this dataset via the st_bbox() function
st_bbox(counties_sf) #spatial command, creates a spatial bounding box around the feature
...
```{r load.geojson.data}
# [we can also download the shapefile and load it in like we did earlier on in this lesson]
# Copy the web address (doesb't load to our harddrive)
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>%  #a lot of datasets will have numeric codes that you don't want to bring in as a "number", keep it as a "character"; we want to read in the FIPS number as a "factor"
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)
 
#(estimate of people in poverty, estimate of people in the minority)
...
counties_sf_join <-  merge(x = counties_sf,
                           ...
                           by.x = "GEOID", 
                           by.y = "FIPS" )
  #specify our x and y tables, and list the two common fields --> GEOID & FIPS

#Tidyverse version of the join (another method)
counties_sf_join <- counties_sf %>% 
  left_join(svi2018_nc_raw, by = c("GEOID" = "FIPS") )

#View with mapview
mapview(counties_sf_join, 
        zcol = 'E_POV', #color by people in poverty
        col.regions = brewer.pal(2, 'RdBu')) + #add a color ramp
  mapview(epa_pm25_sites_sf, cex = 'maxPM') #add another map of EPA sites
...
#create a HUC_8 column...specify a string and we want to grab the first 8 to generate HUC_8
#this dataset doesn't have any geographic data
...
state_sf <- counties_sf %>% 
  group_by('STATEFP') %>% 
  summarize(ALAND = sum(ALAND)) #calculate total area of land in NC
