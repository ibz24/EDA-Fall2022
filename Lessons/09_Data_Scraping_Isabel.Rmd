...
webpage
#html: webpages are written in html, that includes instructions on what to show and how to show it, that is read by your browser software and displayed (according to the instructions)
#we want to extract certain components of the webpage
```
...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% #specify the html nodes
  html_text()
#.table tr:nth-child(1) .left:nth-child(2)

the_registrant #variable extracted from the webpage
```
...
df_withdrawals <- data.frame("Month" = rep(1:12), #rep command, creates a sequence of numbers 1-12, corresponds to the months
                             "Year" = rep(2000,12), #make a vector of 12 repeats of the year 2000
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals), #convert to numeric values
                             "Max_Withdrawals_mgd" = as.numeric(max_withdrawals)) #convert to numeric values
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant,
         ...
         Date = my(paste(Month,"-",Year))) #convert month and year objects to a date
#!! --> saying "this refers to a variable"
...
#break URL down into its variables (makes it easy to look through data from different years)
...
#The tags we found via selector gadget...save them as variables
...
scrape.it <- function(the_year, the_facility){
  ...
  #Sys.sleep(1) #uncomment this if you are doing bulk scraping!
    #We are going to be iterating, which could be confusing to the server-->webiste provider could cut off access to their site
    #Contributes to better ethics
...
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility) #Specify the facility (via the variable above)

#OR, use purrr's map function
the_dfs <- map(the_years,scrape.it,the_facility=my_facility)
  #uses the purr library's map function; maps values onto a function
  #give the function and the values you want
  #same as 'lapply'

#Conflate the returned dataframes into a single dataframe (the_dfs)
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) 
  #map2 command allows you to iterate through the lists in parallel; go through the first item in each pair, then go through the second item in each pair...
...
ggplot(df_2020,aes(y = Avg_Withdrawals_mgd, x=Facility_type)) +
  geom_boxplot()
  #Box plot...
  #x = Facility name OR as.factor(Month)
```
...
the_years = c(2018,2019,2020)
  #construct 2 elements lists...it will end up being a lost of 9 values
