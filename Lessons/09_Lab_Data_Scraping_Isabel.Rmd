...
#Control U -->allows us to see the text behind the webpage
```
...
#Set the URL - save web address as a string
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=20&format=rdb'

#Get the data, which starts on line 30
gage_data <- read.table(theURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 
  #read.table is used for reading tab separated data
  #we want to skip the first 29 rows, saying it has a header separated by a tab ('\t')
...
### Exercise: 
> Alter the above code so that it plots the last 24 days of flow for gage 02085070 (Eno River)
...
gage_data2 <- read.table(theURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 
...
#Identify gauge to download; set variable
siteNo <- '02087500' 
...
scode = "00003"  #mean; aggregating the data
...
#Rename columns to something more useful; special function in this data retrieval package
neuse <- renameNWISColumns(neuse); 
colnames(neuse)
  
#Compute annual means
neuse_by_year <- neuse %>% 
  group_by(year = year(Date)) %>% #group by the year
...
```{r}
#census=get all the data, survery=get a sample
#Apply the API key (ethical; key allows them to track where the service requests are coming from)

#installs the key into a file on the container
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22", install = TRUE)

#Get american community census data
nc_pop <- 
  get_acs(geography = "county", #what level
          variables = "B01003_001", #reference the code lists to get the 
          state = "NC",
          geometry = TRUE,
...
#2&3 Locate elements and read their text attributes into variables-->"look for the text tag on the website"
...
#3. Construct a dataframe from the values
 energydata <- data.frame(States = the_states, 
                        Price = the_price,
...
total_retail <- the_website %>% 
 ...
 energydata <- data.frame(States = the_states, 
                        ...
                        Capacity = the_capacity, 
                        Net_Generation = net_generation,
                        Total_Retail = total_retail) %>% mutate(year = the_year)
